...
```bash
git clone https://github.com/webrek/laravel-money
cd laravel-money
composer install
```
...
- Money is integer-only internally; never introduce float arithmetic into the
  core. Rounding happens only where the caller asks for it.
